...
Write-Host "  ♻️  DESPLIEGUE MANUAL BACKEND RECICLAGO -> AWS ECR      " -ForegroundColor Cyan
Write-Host "==========================================================" -ForegroundColor Cyan
...
Write-Host "`n🔑 Autenticando Docker en Amazon ECR ($REGISTRY)..." -ForegroundColor Yellow
aws ecr get-login-password --region $REGION | docker login --username AWS --password-stdin $REGISTRY

# 1. Asegurar creación de repositorios en ECR
$repos = @("reciclago/ms-bff", "reciclago/ms-catalog", "reciclago/ms-pickups", "reciclago/ms-routes")
foreach ($repo in $repos) {
    Write-Host "🛠️ Verificando repositorio ECR: $repo..." -ForegroundColor Yellow
...
Write-Host "`n🏗️ [1/4] Compilando y subiendo ms-bff..." -ForegroundColor Yellow
docker build -t "$REGISTRY/reciclago/ms-bff:latest" ./ms-reciclago-bff
docker push "$REGISTRY/reciclago/ms-bff:latest"

# 3. Construir y Subir Catálogo
Write-Host "`n🏗️ [2/4] Compilando y subiendo ms-catalog..." -ForegroundColor Yellow
...
Write-Host "`n🏗️ [3/4] Compilando y subiendo ms-pickups..." -ForegroundColor Yellow
...
Write-Host "`n🏗️ [4/4] Compilando y subiendo ms-routes..." -ForegroundColor Yellow
...
Write-Host "  ✅  TODAS LAS IMÁGENES PUBLICADAS EXITOSAMENTE EN ECR   " -ForegroundColor Green
Write-Host "==========================================================" -ForegroundColor Green
Write-Host "`nPara levantar los servicios en tu máquina o en EC2 ejecuta:"
